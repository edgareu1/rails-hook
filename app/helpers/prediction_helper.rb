module PredictionHelper
  ...
  class Predictor
    ...
    # Method that returns the prediction of weight of fish to be caught
    # (per hour) at a certain Location
    def predict(location)
      ...
      location.logs.each do |log|
        ...
        # Analysing "temperature", "air pressure", "wind speed" and "moon"
        x_data.push([
          log.temperature,
          log.air_pressure,
          log.wind_speed,
          log.moon_phase.round(2)
        ])
      end
      ...
      return { weather: weather_data,
        prediction: {
          weight_gr_hour: prediction_weight,
          mean_percentage_error: prediction_mean_error
        }
      }
    end

    # Method that returns the Locations with the highest prediction of weight
    ...
    def top_locations(num)
      @locations.map { |loc| { location: loc }.merge(predict(loc)) }
        .max_by(num) { |loc| [
          loc[:prediction][:weight_gr_hour],
          -loc[:prediction][:mean_percentage_error]
        ]}
